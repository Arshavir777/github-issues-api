...
export class GithubController {
  ...
  @ApiOperation({
    summary: 'Get paginated GitHub issues with total count for a repository',
  })
  @ApiParam({
    name: 'username',
    type: String,
  ...
  @ApiResponse({
    status: 200,
    description: 'List of GitHub issues and pagination info',
  ...
  async getIssues(
    ...
    const result = await this.githubService.getIssuesByRepo(
      username,
      repository,
      page ?? 1,
      perPage ?? 10,
  ...
  @ApiResponse({
    status: 200,
    description: 'The GitHub issue details',
    type: GithubIssueDto,
  })
  @ApiResponse({ status: 400, description: 'Bad Request' })
  ...
  async getSingleIssue(
    ...
    const result = await this.githubService.getIssueByRepo(
      username,
      repository,
      issueNumber,
    );
